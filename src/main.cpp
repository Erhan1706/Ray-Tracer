        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Features", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (config.features.enableSoftShadow) {
                    ImGui::SliderInt("Samples", &numOfSamples, 10, 1000);
                    getNumSamples(numOfSamples);
                }
                ...
                ImGui::Checkbox("Normal interpolation debug", &config.features.enableNormalInterpDebug);
            }
            ImGui::Separator();
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableBloomEffect) {
                    ImGui::SliderFloat("Threshold", &threshold, 0.2f, 0.9f);
                    ImGui::SliderInt("Size of box filter", &boxFilterSize, 1, 6);
                    ImGui::Checkbox("Bloom threshhold", &bloomThreshold);
                    ImGui::Checkbox("Box filter", &boxFilterBool);
                }
                ...
                if (config.features.extra.enableMipmapTextureFiltering) {
                    ImGui::Checkbox("MipMapDebug", &debugMipMap);
                    setDebugMipMap(debugMipMap);
                    if (debugMipMap) {
                        ImGui::SliderInt("MipMap Levels", &mipMapLevel, 0, 7);
                        getMipMapLevel(mipMapLevel);
                    }
                }
                ...
                if (&config.features.extra.enableDepthOfField) {
                    ImGui::SliderFloat("Focal Distance", &camera.m_focal_distance, 1, 10);
                }

                if (&config.features.extra.enableDepthOfField) {
                    ImGui::SliderFloat("Aperture", &camera.m_aperture, 0, 1);
                }


                ImGui::Checkbox("Antialising", &config.features.extra.enableMultipleRaysPerPixel);
                ImGui::Checkbox("Motion Blur", &config.features.extra.enableMotionBlur);
                if (config.features.extra.enableMotionBlur) {
                    ImGui::Checkbox("Motion Blur Debug", &motionDebug);
                    ImGui::SliderInt("Samples", &samplesMotionBlur, 10, 50);
                    ImGui::SliderFloat("Translation - X", &translationX, -2.0f, 2.0f);
                    ImGui::SliderFloat("Translation - Y", &translationY, -2.0f, 2.0f);
                    ImGui::SliderFloat("Translation - Z", &translationZ, -2.0f, 2.0f);
                }
            }
            ImGui::Separator();
            ...
            case ViewMode::Rasterization: {
                ...
                if (optDebugRay) {
                    ...
                    if (config.features.extra.enableDepthOfField) {
                        debug_dof(scene, *optDebugRay, camera, bvh, screen, config.features);
                    }
            ...
            case ViewMode::RayTracing: {
                ...
                if (config.features.extra.enableMotionBlur) {
                    std::vector<glm::vec3> finalColors = screen.pixels();
                    std::mt19937 rng;
                    // Random variable uniformly distributed between 0 and 1.
                    std::uniform_real_distribution<float> dist(0, 1);
                    // The number of samples determines how many renders we will have with the object in different positions.
                    for (int i = 0; i < samplesMotionBlur; i++) {
                        renderRayTracingMotion(scene, camera, bvh, screen, config.features,dist(rng), motionDebug, translationX, translationY, translationZ);
                        for (int j = 0; j < screen.pixels().size(); j++) {
                            finalColors[j] += screen.pixels()[j];
                        }
                    }
                    for (int i = 0; i < screen.pixels().size(); i++) {
                        screen.setPixel(i, finalColors[i] / (float) samplesMotionBlur);
                    }
                    
                } else 
                    renderRayTracing(scene, camera, bvh, screen, config.features);
                std::vector<glm::vec3> oldPixelValues = screen.pixels();
                //Apply bloom filter
                if (config.features.extra.enableBloomEffect) {
                    // Threshold bright pixels.
                    for (int i = 0; i < screen.pixels().size(); i++) {
                        glm::vec3 pixel = screen.pixels()[i];
                        if (pixel.x < threshold && pixel.y < threshold && pixel.z < threshold) 
                            screen.setPixel(i, glm::vec3(0.0f));
                    }
                    // Apply box filter to the image.
                    if ((config.features.extra.enableBloomEffect || boxFilterBool) && !bloomThreshold) {
                        for (int i = 0; i < screen.pixels().size(); i++) {
                            glm::vec3 newColor = screen.boxFilter(screen.pixels()[i], i, boxFilterSize);
                            if (boxFilterBool) {
                                // If we want to see the full bloom effect add to original colors.
                                screen.setPixel(i, newColor);
                            } else {
                                // Otherwise just set the color to the color returned after the box effect.
                                screen.setPixel(i, oldPixelValues[i] + newColor);
                            }
                        }
                    }
                }

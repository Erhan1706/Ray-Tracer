...
static std::ostream& operator<<(std::ostream& os, const SceneType& sceneType)
{
    ...
    case SceneType::TransparentDebug: {
        os << "SceneType::TransparentDebug";
        break;
    }
    case SceneType::Mipmap: {
        os << "SceneType::Mipmap";
        break;
    }
    }
...
std::ostream& operator<<(std::ostream& os, const Config& config)
{
    ...
    os << "  + output_filepath: " << config.outputDir << std::endl
       ...
       << "    - enable_normal_interp_debug: " << config.features.enableNormalInterpDebug << std::endl
...
Config readConfigFile(const std::filesystem::path& config_path)
{
    ...
    config.features.enableAccelStructure = table["features"]["enable_accel_structure"]
                                       .as_boolean()
                                       ->value_or(false);
    config.features.enableNormalInterpDebug = table["features"]["enable_normal_interp_debug"]
                                             .as_boolean()
                                             ->value_or(false);
...
std::optional<SceneType> deserialize(const std::string& sceneTypeStr)
{
    ...
    if (lowered == "single_triangle" || lowered == "singletriangle" || lowered == "single-triangle") {
        ...
        return SceneType::TransparentDebug;
    } else if (lowered == "Mipmap") {
        return SceneType::Mipmap;
    }
    else {
        return std::nullopt;
    }

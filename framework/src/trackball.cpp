...
// Give each ray a random time that makes it see the mesh in different positions - simulating motion.
Ray Trackball::generateRayMotion(const glm::vec2& pixel, float time, float translationX, float translationY, float translationZ) const
{
    const glm::vec3 cameraSpaceDirection = glm::normalize(glm::vec3(-pixel.x * m_halfScreenSpaceWidth, pixel.y * m_halfScreenSpaceHeight, 1.0f));

    Ray ray;
    // Offset the ray according to the time RV, essentially moving the view.
    ray.origin = { position().x + translationX * time, position().y + translationY * time, position().z + translationZ * time };
    ray.direction = glm::quat(m_rotationEulerAngles) * cameraSpaceDirection;
    ray.t = std::numeric_limits<float>::max();
    return ray;
}
